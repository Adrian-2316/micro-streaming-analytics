...
public class Statistics {
    ...
    public Statistics calculate(List<Datapoint> datapointList) {
        List<Double> values = datapointList.stream().map(Datapoint::getValue).collect(Collectors.toList());
    ...
    public void calculateStandardDeviation(List<Double> values) {
        ...
        standardDeviation = values.stream().mapToDouble(num -> num).map(num -> Math.pow(num - mean, 2)).sum();
